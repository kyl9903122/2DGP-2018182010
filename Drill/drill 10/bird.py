...
class BIRD:
    ...
    def draw(self):
        print("enter draw")
        if self.dir == 1:
            self.image.clip_draw((int(self.frame) % 5) * 182, (2-int(self.frame // 5)) * 165, 183, 165, self.x, self.y, 180,
                                 160)
            print(int(self.frame) % 5, int(self.frame // 5))
        else:
            self.image.clip_composite_draw(int(self.frame % 5) * 182, (2-int(self.frame / 5) )* 165, 183, 165, 0, 'h',
